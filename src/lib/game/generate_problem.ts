...
function chooseOperation(operations: string[]){
...
export function generateProblem(problemType: string, operations: string[]){
    ...
    if (operation === '+'){
        ...
        return {'operation': '+', 'firstNum': first, 'secondNum': sec, 'answer': first + sec, 'statement': String(first) + ' + ' + String(sec) + ' = '};
    }
    else if (operation === '-'){
        let first = randomInt(bounds[problemType]['-']['first'][0], bounds[problemType]['-']['first'][1]);
        let sec = randomInt(bounds[problemType]['-']['second'][0], bounds[problemType]['-']['second'][1]);
        return {'operation': '-', 'firstNum': first + sec, 'secondNum': sec, 'answer': first, 'statement': String(first + sec) + ' - ' + String(sec) + ' = '};
    }
    else if (operation === '*'){
        let first = randomInt(bounds[problemType]['*']['first'][0], bounds[problemType]['*']['first'][1]);
        let sec = randomInt(bounds[problemType]['*']['second'][0], bounds[problemType]['*']['second'][1]);
        ...
        return {'operation': '/', 'firstNum': first, 'secondNum': sec, 'answer': Math.floor(first / sec), 'statement': String(first) + ' / ' + String(sec) + ' = '};
    }
}
